...
"""
In-sample solutions for a method variant (Benders + full CG)
...
function main()
    ...
    ioStream = open(joinpath(output_dir, string(trial_id) * "_Met_BFCG.csv"), "w")
    ...
    write(ioStream, join([trial_id, "BFCG", LB, UB, alg_time, it, num_cuts, num_vars, precomp_time_sp, rootnode_time, MP_time, SP_time, enumeration_time], ",") * "\n")
