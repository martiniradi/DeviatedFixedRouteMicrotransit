...
"""
In-sample solutions for a method variant (Benders + full enumeration)
...
function main()
    ...
    ioStream = open(joinpath(output_dir, string(trial_id) * "_Met_BENUM.csv"), "w")
    ...
    #--- Benders + full enumeration (!CG, !HEUR)
    (all_subpaths, subpath_road_networks, all_load_expanded_graphs, all_subpath_graphs, enumeration_time), precomp_time_sp = @timed generateSubPathSet(m, R, inst, !CG, !TRANSIT);
    ...
    write(ioStream, join([trial_id, "BENUM", LB, UB, alg_time, it, num_cuts, num_vars, precomp_time_sp, rootnode_time, MP_time, SP_time, enumeration_time], ",") * "\n")
